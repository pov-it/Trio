...
4. Create a `CKShare` on that root when no validated invite URL is stored (`ai_meal_companion_share_url`). `MealFeed` is not indexable and `Meal.recordName` is not queryable, so Trio loads `MealFeedRoot` with `record(for:)` — the dashboard record list can be empty while the root and a share still exist. If `share` is set but `CKShare.url` is missing or not an `https` iCloud link, Trio deletes that share and, only if the root’s share reference remains, replaces `MealFeedRoot` (same record name; child `Meal` parent ids stay). It then saves the root and a new `CKShare` together (`changedKeys`, one retry with `ifServerRecordUnchanged`, `publicPermission` readOnly). A `CKError` from that delete or save is shown as `CKError <code> (<name>): <message>`, not the idle “No invite link yet…” line. A root with no share reference is not deleted.
